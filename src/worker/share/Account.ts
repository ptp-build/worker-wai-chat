...
export default class Account {
	static UserIdAccountIdMap: Record<string, Account[]> = {};
	...
	async setEntropy(entropy: string) {
		...
		const key = sha256(Buffer.from(`${KEY_PREFIX}${this.getAccountId()}`)).toString('hex');
		let cipher = encrypt(
			Buffer.from(entropy, 'hex'),
			Buffer.from(key.substring(0, 16)),
			Buffer.from(key.substring(16, 32))
		);
		await Account.getKv().put(`${KEY_PREFIX}${key}`, cipher.toString('hex'));
	}
